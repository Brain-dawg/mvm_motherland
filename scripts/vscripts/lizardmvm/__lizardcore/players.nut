...
::CTFBot.CollectWeaponsAndCosmetics <- CTFPlayer.CollectWeaponsAndCosmetics;

::CTFPlayer.CollectAttachments <- function()
{
    local items = [];
    local extraVM;
    for (local item = FirstMoveChild(); item != null; item = item.NextMovePeer())
...
::CTFBot.CollectAttachments <- CTFPlayer.CollectAttachments;
...
::CTFPlayer.CollectWeapons <- function()
{
    local result = [];
	for (local i = 0; i < MAX_WEAPON_COUNT; i++)
	{
