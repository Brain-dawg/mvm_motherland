...
::CollectByClassname <- function(classname)
{
    local entities = [];
...
::CollectByClassnameWithin <- function(classname, origin, radius)
{
    local entities = [];
...
::CollectByName <- function(targetname)
{
    local entities = [];
...
if (!("PrecacheSoundOriginal" in ROOT))
    ::PrecacheSoundOriginal <- ::PrecacheSound; //todo tmp hack
::PrecacheSound <- function(path)
{
    OnTickEnd(PrecacheSoundOriginal, path);
}
