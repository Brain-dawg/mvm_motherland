...
if ( !( "_Motherland_EntAdditions" in ROOT ) ) {

    ::_Motherland_EntAdditions <- class {
    ...
    __CollectGameEventCallbacks( _Motherland_EntAdditions.Events )
}
...
function Precache() {
    ...
    if ( classname in _Motherland_EntAdditions.Precache )
        _Motherland_EntAdditions.Precache[classname]( self, spawnflags )
}
...
function OnPostSpawn() {
    ...
    local classname = self.GetClassname()
    local spawnflags = GetPropInt( self, "m_spawnflags" )
    if ( classname in _Motherland_EntAdditions.OnPostSpawn )
        _Motherland_EntAdditions.OnPostSpawn[classname]( self, spawnflags )
}
