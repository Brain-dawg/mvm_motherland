...
AddOutput( _MotherlandMain.gateB, "OnCapTeam2", "player", "RunScriptCode", "if ( self.IsBotOfType( TF_BOT_TYPE ) && self.HasBotAttribute( AGGRESSIVE|IGNORE_FLAG ) && !self.HasBotTag( `tag_alwayspush` ) ) self.RemoveBotAttribute( AGGRESSIVE|IGNORE_FLAG )", 0, -1 )

// additional changeattributes for individual gates
AddOutput( _MotherlandMain.gateA, "OnCapTeam2", "point_populator_interface", "ChangeBotAttributes", "GateACapped", 0, -1 )
AddOutput( _MotherlandMain.gateB, "OnCapTeam2", "point_populator_interface", "ChangeBotAttributes", "GateBCapped", 0, -1 )

local altbomb = _MotherlandMain.AltBomb
altbomb.ValidateScriptScope()
local altbomb_scope = altbomb.GetScriptScope()

// Fake bomb for HUD icon
altbomb_scope.FakeBomb <- _MotherlandUtils.FakeBomb
